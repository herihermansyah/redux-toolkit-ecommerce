...
const ProductList: React.FC = () => {
  ...
  // Ambil products & categories dari Redux store
  const products = useSelector<RootState, Product[]>(selectFilteredProducts);
  const categories = useSelector<RootState, string[]>(selectCategories);
  ...
  const handleCategoryChange = (cat: string) => {
    setSelectedCategory(cat); // update dropdown lokal
    dispatch(filterByCategory(cat)); // update Redux filter
  };
  ...
  return (
    <div className="container mx-auto p-4">
      {/* Filters */}
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-2 sm:gap-4">
        {/* Category Dropdown */}
        <select
          value={selectedCategory}
          onChange={(e) => handleCategoryChange(e.target.value)}
          className="px-2 py-1 sm:px-4 sm:py-2  shadow-sm 
